...
def evaluate_predicate_prediction(model, data_test, len_test,
                                  labels, thres_lists):
    num_test_instance = 0
...
def add_results_for_predicate_prediction(label, p_ys, result, thres_list, ys):
    for i in range(len(p_ys)):
        y = ys[i]
        p_y = p_ys[i]
        for thres in thres_list:
            prob = math.pow(math.e, p_y.data[0]) - thres
